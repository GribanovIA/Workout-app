    ...
    >
      <b-card-body>
        <div v-for="(set, index) in exercise.sets" class="set">
          ...
          <b-card-text>
            Вес:
            <span class="new-value">{{ set.new.weight }} кг</span> /
            <span class="old-value">{{ set.old.weight }} кг</span>
            <br>Кол-во повторений:
            <span class="new-value">{{ set.new.reps }}</span> /
            <span class="old-value">{{ set.old.reps }}</span>
            <br>
          </b-card-text>
        </div>
      </b-card-body>
...
export default {
    ...
    computed: {
        ...
        workoutName() {
            return this.workoutsHistory[this.$route.params.date].workoutName;
        },
        exercises() {
